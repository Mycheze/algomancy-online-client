...
export const CARD_TODO: TodoEntry[] = [
  ...
  },
  {
    ...
    detail:
      ...
      + 'five cards of the deck and cache one. Until end of turn, you may play it as if it was in '
      + 'your hand, ignoring affinity)" — and stops. Premonition, Celestial Purge, Dematerialize '
      + 'and Foretell all read the same way. The ENGINE recycles the rest to the bottom of the '
      + 'deck (R45 as corrected 2026-08-19) and its log line says so out loud, so the game is '
      + 'right and the card is silent about half of what it does.',
    evidence:
    ...
    fix:
      '⚠ THIS IS A DATA FIX UPSTREAM AND THAT IS THE WHOLE DIFFICULTY. `printed.json` is '
      + 'GENERATED by `scripts/extract-printed.mjs` from `AlgomancyCards-OracleText.json`, so a '
      + 'hand edit to printed.json is wiped on the next regeneration. Same shape as Might of the '
      + 'Grove, Interdiction Rift and Arbiter of Armistice\'s {Switch}. '
      + 'The RAG corpus and the Discord bot read the ORACLE file DIRECTLY, so they are wrong '
      + 'about this too — one message to Caleb fixes all three places; an extractor override '
