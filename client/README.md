...
| doc | what |
|---|---|
| [`digital-rules.md`](docs/digital-rules.md) | **the engine's spec.** Every adjudication the paper rules forced, R1–R302 and counting, each with its source and the test that encodes it. One `## R<n>` heading per ruling; `engine/test/184-ruling-register.test.ts` checks the register against the code |
