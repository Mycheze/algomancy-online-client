...
/** The card transcription: Caleb's for the base set, ours for Light & Dark.
 *  CANONICAL — a wrong field is corrected here, in place, with the reason in
 *  the commit; the generated files below are rebuilt from it. */
...
/**
 * GENERATED by `bot/pipeline/build_anchors.py`: where each mod card's
 * graft/augment icon sits on its scan, and `cut`, the gap above the line it
 * opens. The bot slices its stacked-card images there; `extract-printed.mjs`
 * turns it into the catalogue's `modPeek`, so the client's mod strip under a
 * host shows the same slice. ⚠ Generated — never hand-edit it.
 */
export const MOD_ANCHORS = join(CARDS_DIR, 'mod_anchors.json');
