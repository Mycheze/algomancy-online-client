...
This package is the engine: `src/` is the pure reducer, `test/` is its suite,
`scripts/` is the build tooling. The browser client is the sibling package
`../ui/` (it was `../ui/` in here until 2026-08-30) and its tests are in `../ui/test/`.
...
**No card is parked.** `CARD_LEDGER` (`../ledgers/card-ledger.ts`), which declares
...
cannot be built yet goes in `../ledgers/card-ledger.ts` (a dead card half) or
`../ledgers/card-todo.ts` (anything else) — both are checked against reality.
