 ...
 *
 * `client/server/` had no `tsconfig.json`. Not a lax one — none at
...
test('§2 the root `check` script invokes every tsconfig project', () => {
  ...
  assert.ok(leaves.length >= 3,
    ...
    + 'from client/package.json or the script-graph walk stopped following npm run');
  ...
  for (const c of cfgs) {
    ...
    for (const { flag, why } of REQUIRED_FLAGS) {
      assert.equal(c.options[flag], true,
        ...
        + 'Every project under client/ is checked to the same standard on\n'
