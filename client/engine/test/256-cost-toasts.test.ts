...
import { effectByKey, type EffectCtx, type ResolvedTarget } from '../src/cards/dsl.ts';
...
import { HOLD_MS, STAGGER_MS } from '../../ui/flash.ts';
import type { EngineEvent, EventType, GameState } from '../src/types.ts';
...
test('[R276] the classifiers are the same ones 244 derives the inventory with', () => {
  ...
  const theirs = read('test/244-log-is-not-the-only-surface.test.ts');
...
test('[R276] this scan and the one in 244 agree about how many announcements the engine makes', () => {
  const src = read('test/244-log-is-not-the-only-surface.test.ts');
