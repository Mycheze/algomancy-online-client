...
import { apply, legalActions, IllegalAction } from '../../engine/src/apply.ts';
import { optionKeys } from '../inspect.ts';
import type { AutoPassArm } from '../inspect.ts';
import {
  armSnapshot, attackFrom, canJoinFormation, formationCandidates, inPassWindow, passAllRelease,
} from '../battle.ts';
import { giveResources, pass, spawn, toDeployment, toNextBattle } from '../../engine/test/util.ts';
...
const unitIdsOf = (s: GameState, seat: Seat): EntityId[] =>
...
test('[127] the attack affordance names exactly the units the engine would take', () => {
  ...
  const engineWould = unitIdsOf(s, D)
...
test('[127] the block step is the same question, asked of the defender', () => {
  ...
  const engineWould = unitIdsOf(s, D).filter(id =>
