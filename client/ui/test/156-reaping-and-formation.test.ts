...
import { AUTHORED_GLOSSARY } from '../glossary.ts';
import type { EntityId, Seat, TargetRef } from '../../engine/src/types.ts';
...
test('the KILL-scoped attribute set is DERIVED from the pool wording, not hardcoded (R184)', () => {
  ...
  const printed = JSON.parse(fs.readFileSync(
    path.join(HERE, '..', '..', 'engine', 'src', 'cards', 'printed.json'), 'utf8')) as
