...
import { blockDeclarationIssue, legalActions } from '../../engine/src/apply.ts';
import { blockVerdict } from '../battle.ts';
import type { Action, EntityId, GameState, Seat } from '../../engine/src/types.ts';
import { finishBattle, pass, pick, spawn, toDeployment, toNextBattle } from '../../engine/test/util.ts';
import { client, idsIn, zone } from './ui-driver.ts';

const MAIN = readFileSync(fileURLToPath(new URL('../main.ts', import.meta.url)), 'utf8');
const BATTLE = readFileSync(fileURLToPath(new URL('../battle.ts', import.meta.url)), 'utf8');
