...
import { allCardNames, ambushEffect, getCard, specForSlot } from '../../engine/src/cards/dsl.ts';
...
import type { ActivatedAbility, EffectDef, TargetSpec } from '../../engine/src/cards/dsl.ts';
import { Harness } from '../../engine/src/harness.ts';
import type { Action, Entity, EntityId, Seat, TargetRef } from '../../engine/src/types.ts';
import { ent, giveResources, spawn, toDeployment, toNextBattle, withE } from '../../engine/test/util.ts';
...
const PRINTED = JSON.parse(readFileSync(
  fileURLToPath(new URL('../../engine/src/cards/printed.json', import.meta.url)), 'utf8',
...
test('R265 §3e every ally and enemy list a card builds for itself names a region', () => {
  ...
  const dir = fileURLToPath(new URL('../../engine/src/cards/', import.meta.url));
  const paths = [
